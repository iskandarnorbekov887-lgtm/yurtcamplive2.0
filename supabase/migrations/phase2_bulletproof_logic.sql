...
ALTER TABLE inventory ADD COLUMN IF NOT EXISTS buy_unit TEXT;
ALTER TABLE inventory ADD COLUMN IF NOT EXISTS conversion_factor FLOAT DEFAULT 1.0;
-- Rename unit_type to use_unit if not already done, or just use it as use_unit
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='inventory' AND column_name='unit_type') THEN
    ALTER TABLE inventory RENAME COLUMN unit_type TO use_unit;
  END IF;
END $$;
