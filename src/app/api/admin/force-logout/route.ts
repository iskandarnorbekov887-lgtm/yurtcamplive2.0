...
export async function POST(request: Request) {
  try {
    ...
    if (callerProfile.role !== 'CEO') {
      return NextResponse.json(
        { error: 'Forbidden: Only CEO can force logout users' },
        { status: 403 }
      );
    }
    ...
    if (targetProfile.team_id !== callerProfile.team_id) {
      return NextResponse.json(
        { error: 'Forbidden: Cannot force logout user from another team' },
        { status: 403 }
      );
    }
    ...
    const { error: logoutError } = await adminClient.auth.admin.signOut(targetUserId, 'global');

    if (logoutError) {
      return NextResponse.json(
        { error: `Failed to force logout: ${logoutError.message}` },
        { status: 500 }
      );
    }
    ...
    console.error('Error in force-logout route:', error);
    return NextResponse.json(
