...
export function ManagerProcurement() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-zinc-950 p-8">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="flex justify-between items-end">
          <div>
            ...
            <p className="text-slate-400 font-black tracking-[0.2em] text-[10px] uppercase mt-1">Multi-Currency Procurement & Inventory Audit</p>
          </div>
          <div className="bg-white border border-black px-4 py-2 rounded-none flex items-center gap-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            <div className="w-2 h-2 bg-emerald-500 rounded-full" />
            <span className="text-[10px] font-black uppercase tracking-widest text-black">Status: Auditor Active</span>
          </div>
        </div>
          ...
          {requests.map((req) => {
            ...
            return (
              <motion.div 
                key={req.id}
                layout
                className="bg-white border border-black rounded-none overflow-hidden relative shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
              >
                <div className="absolute top-4 right-4 z-10">
                  <span className={`px-2.5 py-1 border border-black text-[9px] font-black uppercase tracking-widest ${
                    req.status === 'sent' ? 'bg-amber-400 text-black' :
                    req.status === 'finalized' ? 'bg-emerald-500 text-white' :
                ...
                <div className="p-8">
                  <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-8">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 border border-black flex items-center justify-center bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                      ...
                      <div className="bg-[#FFFFFF] border border-black p-6 w-full lg:w-[450px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] grid grid-cols-12 gap-4">
                        ...
                        <div className="col-span-6">
                           ...
                           <input 
                             ...
                             className="w-full bg-white border border-black px-3 py-2 font-mono text-base font-black text-black outline-none focus:bg-zinc-50"
                           />
                        </div>
                        ...
                        <div className="col-span-6">
                           ...
                           <select 
                             ...
                             className="w-full bg-white border border-black px-3 py-2 font-mono text-base font-black text-black outline-none appearance-none"
                           >
                        ...
                        <div className="col-span-12 pt-4 border-t border-black mt-2">
                           <div className="flex justify-between items-baseline">
                              <span className="text-[10px] font-black uppercase text-black">Converted Total:</span>
                              <span className="text-2xl font-mono font-black text-black">{totalUzs.toLocaleString()} UZS</span>
                           </div>
                        </div>
                      </div>
                    ) : (
                      <div className="bg-emerald-50 border border-black p-6 w-full lg:w-[450px] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                         <p className="text-[10px] font-black uppercase tracking-widest text-emerald-700 mb-4">Finalized Financial Record</p>
                         <div className="flex justify-between items-baseline mb-2">
                            <span className="text-[10px] font-black text-black uppercase">Spent (UZS):</span>
                            <span className="text-2xl font-mono font-black text-black">{(req.total_spent_uzs || 0).toLocaleString()} UZS</span>
                         </div>
                         <p className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter font-mono">
                  ...
                  <div className="space-y-4">
                    {req.procurement_items?.map((item) => (
                      <div key={item.id} className="grid grid-cols-12 gap-4 bg-white border border-black p-5 hover:translate-x-[1px] hover:translate-y-[1px] transition-all items-center">
                        <div className="col-span-4 flex items-center gap-3">
                          <div className="w-10 h-10 border border-black flex items-center justify-center font-black text-black text-sm bg-zinc-50">
                            {item.inventory?.item_name[0]}
                          </div>
                          <div>
                            <p className="font-black text-black text-sm uppercase tracking-tight">{item.inventory?.item_name}</p>
                            <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest">{item.inventory?.buy_unit}</p>
                          </div>
                        </div>
                        ...
                        <div className="col-span-3">
                          <p className="text-[9px] text-black font-black uppercase tracking-widest mb-1.5">Inbound ({item.inventory?.buy_unit})</p>
                          <input 
                            type="number"
                            disabled={isFinalized}
                            value={item.actual_received_qty ?? ''}
                            onChange={(e) => updateItemField(item.id, 'actual_received_qty', parseFloat(e.target.value) || 0)}
                            className="w-full bg-white border border-black py-2 px-3 text-center font-mono text-lg font-black text-black outline-none disabled:bg-zinc-100"
                          />
                        </div>

                        <div className="col-span-3">
                          <p className="text-[9px] text-black font-black uppercase tracking-widest mb-1.5">Unit Ref ($)</p>
                          <div className="relative">
                            ...
                            <input 
                              ...
                              className="w-full bg-white border border-black py-2 pl-8 pr-3 font-mono text-lg font-black text-black outline-none disabled:bg-zinc-100"
                            />
                          </div>
                        </div>
                      </div>
      ...
      <div className="fixed bottom-10 right-10 z-50">
        <motion.div 
          ...
          className="bg-white border-2 border-black p-8 rounded-none shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col gap-4"
        >
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-black pb-1 mb-2">Daily Audit</p>
            <div className="flex items-baseline gap-4">
               <span className="text-[10px] font-black text-black uppercase">Finalized Total:</span>
